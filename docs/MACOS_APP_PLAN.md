...
### Phase 10: App Icon + DMG + GitHub Release ✅ COMPLETE

**Goal:** A polished drag-to-install `.dmg` with app icon that friends can download from a single link.

**Implementation steps:**

1. **App icon** — generated by `scripts/generate-assets.py` (Python + Pillow, no external tools):
   - Amber gradient background (#F59E0B → #B45309, top-to-bottom)
   - Three stacked white photo-print rectangles (rotated ±8–12°) with a warm sun glow
   - All 10 required macOS sizes: 16, 32, 128, 256, 512 px @1x and @2x
   - Output: `mac-app/SundayAlbum/Assets.xcassets/AppIcon.appiconset/`
   - `project.yml`: added `ASSETCATALOG_COMPILER_APPICON_NAME: AppIcon`

2. **DMG background** — also generated by `generate-assets.py`:
   - 660×400 logical (1320×800 @2x Retina), warm cream gradient with amber glow
   - "Sunday Album" title, drag instruction, amber arrow from app zone → Applications zone
   - Output: `mac-app/assets/dmg-background.png` + `dmg-background@2x.png`

3. **`build-release.sh`** — updated to add `create-dmg` step after `.app` export:
   ```bash
   create-dmg \
     ...
     "SundayAlbum-1.0-beta1.dmg" \
     "SundayAlbum.app"
   ```
   Falls back to `.zip` if `create-dmg` is not installed. Run: `cd mac-app && ./build-release.sh`

4. **GitHub Release** — create via `gh release create`:
   ```bash
   gh release create v1.0-beta1 \
     ~/Desktop/SundayAlbum-1.0-beta1.dmg \
     --title "Sunday Album 1.0 beta 1" \
...
- `scripts/generate-assets.py` — generates icon PNGs + DMG background in one pass; run after any brand change
- `mac-app/SundayAlbum/Assets.xcassets/AppIcon.appiconset/` — 10 icon PNGs + `Contents.json`
- `mac-app/assets/dmg-background.png` + `dmg-background@2x.png`
- `mac-app/build-release.sh` — updated to produce `.dmg` via `create-dmg` (falls back to `.zip`)
- `mac-app/project.yml` — added `ASSETCATALOG_COMPILER_APPICON_NAME: AppIcon`

**Gatekeeper bypass instructions for beta recipients** (unsigned app, no Apple Developer account):
```
...
**System requirements for beta invite:**
```
macOS 15 Sequoia or later (macOS 26 Tahoe supported)
Apple Silicon Mac (M1/M2/M3/M4)
Python 3.12+ installed (via Homebrew or Xcode tools)
Anthropic API key (Claude) — free tier works
OpenAI API key — optional, enables higher-quality glare removal
~500 MB disk space (app installs Python deps on first launch)
~2–3 min internet connection on first launch (pip install)
```

**Deliverable:** ✅ `cd mac-app && ./build-release.sh` produces `~/Desktop/SundayAlbum-1.0-beta1.dmg` with the app icon, drag-to-Applications window, and warm branded background. Upload to a GitHub Release as a pre-release asset.

---

### Phase 11: Code Signing + Notarization *(deferred — not required for v1 beta)*

**Can friends bypass the warning without notarization?** Yes — for a small beta group, the unsigned app works fine. Include the Gatekeeper bypass instructions (above) in the beta invite.

**When notarization becomes necessary:**
- Distributing to non-technical users who won't know how to bypass
- More than ~10–20 beta testers
...
3. Archive → Distribute App → Developer ID from Xcode, then:
   ```bash
   xcrun notarytool submit SundayAlbum.dmg --apple-id you@email.com \
...
[x] Phase 9: New user (no .venv nearby) sees SetupView on first launch
[x] Phase 9: Live pip log streams line-by-line with progress bar during setup
[x] Phase 9: Cancel aborts install and deletes partial venv; app returns to NeedsSetup state
[x] Phase 9: Setup completes → smooth transition to library via .animation
[x] Phase 9: Help menu → "Uninstall Sunday Album…" removes all data and quits
[x] Phase 9: build-release.sh produces a zip on Desktop in one command
[x] Phase 10: App icon visible in Dock and Finder (amber gradient + stacked photos motif)
[x] Phase 10: build-release.sh produces SundayAlbum-1.0-beta1.dmg on Desktop
[x] Phase 10: DMG mounts and shows drag-to-Applications window with branded background
[x] Phase 10: Dragging app to Applications installs it; right-click → Open clears Gatekeeper
```
