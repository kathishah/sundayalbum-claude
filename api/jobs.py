...
def _handle_list(user_hash: str) -> dict:
    ...
    for item in resp.get("Items", []):
        ...
        # Presign thumbnail so card thumbnails render without a separate getJob call
        thumb_key = item.get("thumbnail_key", "")
        if thumb_key:
            try:
                job["thumbnail_url"] = presign_get(thumb_key, expires=3600)
            except Exception:
                pass
        jobs.append(job)
...
def _handle_get(job_id: str, user_hash: str) -> dict:
    ...
    # Presigned URL for original upload (for download / raw access)
    upload_key = item.get("upload_key", "")
    if upload_key:
        enriched["upload_url"] = presign_get_if_exists(upload_key, expires=3600)

    # Card thumbnail (400px JPEG generated by load handler)
    thumb_key = item.get("thumbnail_key", "")
    if thumb_key:
        try:
            enriched["thumbnail_url"] = presign_get(thumb_key, expires=3600)
        except Exception:
            pass
